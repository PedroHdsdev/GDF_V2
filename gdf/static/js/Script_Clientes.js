...
function fn_preencher_formulario(data) {
  ...
  og_estado_clientes.sapConnections = [];
  og_estado_clientes.sapSelectedId = null;

  if (data.sap_connections && Array.isArray(data.sap_connections)) {
    og_estado_clientes.sapConnections = data.sap_connections.slice();
  } else if (data.sap_connection) {
    // Compatibilidade com payload legado
    og_estado_clientes.sapConnections = [data.sap_connection];
  }

  if (og_estado_clientes.sapConnections.length > 0) {
    semRegistro.classList.add('d-none');
    formContainer.classList.remove('d-none');
    const primeiraAtiva = og_estado_clientes.sapConnections.find(c => Boolean(c.active));
    const primeira = primeiraAtiva || og_estado_clientes.sapConnections[0];
    og_estado_clientes.sapSelectedId = String(primeira.id || '');
    fn_renderizar_sap_connections_select();
    fn_selecionar_sap_connection(og_estado_clientes.sapSelectedId);
  } else {
    semRegistro.classList.add('d-none');
    formContainer.classList.remove('d-none');
    fn_renderizar_sap_connections_select();
      ...
      <td class="text-center">
        <div class="form-check form-switch d-flex justify-content-center">
          <input 
            ...
            data-action="alternar-solucao-status"
            data-solucao="${fn_cli_escAttr(sol.cod_solucao)}">
        </div>
      </td>
      <td class="text-center">
        <button 
...
function fn_renderizar_grupos_cliente() {
  ...
  og_estado_clientes.gruposClienteSelecionados.forEach(g => {
    ...
    row.innerHTML = `
      ...
      <td class="text-center">
        <button type="button" class="btn btn-sm btn-danger" data-action="remover-grupo-cliente" data-grupo-id="${g.id}">Remover</button>
      </td>
    `;
    tbody.appendChild(row);
  });
...
/* ===============================
   CONEXÃO SAP – SUPORTE A MÚLTIPLAS CONEXÕES
================================ */

function fn_renderizar_sap_connections_select() {
  const container = document.getElementById('sap-connections-select-container');
  const select = document.getElementById('sap_connections_select');
  if (!container || !select) return;

  if (!og_estado_clientes.sapConnections.length) {
  ...
  select.innerHTML = og_estado_clientes.sapConnections
    .map(conn => {
      const id = String(conn.id || '');
      const tipo = (conn.tipo_conexao || 'RFC').toUpperCase();
      const host = conn.ashost || 'Sem host';
      const ativo = conn.active ? 'Ativa' : 'Inativa';
      const selected = String(og_estado_clientes.sapSelectedId || '') === id ? ' selected' : '';
      return `<option value="${fn_cli_escAttr(id)}"${selected}>${fn_cli_escHtml(host)} - ${fn_cli_escHtml(tipo)} (${fn_cli_escHtml(ativo)})</option>`;
    })
    .join('');
}

function fn_selecionar_sap_connection(sapId) {
  const id = String(sapId || '');
  const conn = og_estado_clientes.sapConnections.find(c => String(c.id || '') === id);
  if (!conn) return;

  og_estado_clientes.sapSelectedId = id;
  document.getElementById('sap_id').value = conn.id || '';
  document.getElementById('sap_tipo_conexao').value = (conn.tipo_conexao || 'RFC').toUpperCase();
  document.getElementById('sap_ashost').value = conn.ashost || '';
  document.getElementById('sap_sysnr').value = conn.sysnr || '';
  document.getElementById('sap_client').value = conn.client || '';
  document.getElementById('sap_username').value = conn.username || '';
  document.getElementById('sap_passwd').value = conn.passwd || '';
  document.getElementById('sap_lang').value = conn.lang || '';
  document.getElementById('sap_active').checked = Boolean(conn.active);
}

function fn_nova_conexao_sap() {
  og_estado_clientes.sapSelectedId = null;
  document.getElementById('sap_id').value = '';
  document.getElementById('sap_tipo_conexao').value = 'RFC';
...
async function fn_submit_sap(form) {
  ...
  const sapId = (document.getElementById('sap_id').value || '').trim();
  const formData = new FormData(form);
  if (sapId) {
    formData.set('sap_id', sapId);
  } else {
    formData.delete('sap_id');
  }
  ...
  try {
    ...
    if (data.success) {
      ...
      if (data.sap_connection) {
        og_estado_clientes.sapSelectedId = String(data.sap_connection.id || '');
        fn_renderizar_sap_connections_select();
        fn_selecionar_sap_connection(og_estado_clientes.sapSelectedId);
      } else if (og_estado_clientes.sapConnections.length) {
        if (!og_estado_clientes.sapSelectedId) {
          og_estado_clientes.sapSelectedId = String(og_estado_clientes.sapConnections[0].id || '');
        }
        fn_renderizar_sap_connections_select();
        fn_selecionar_sap_connection(og_estado_clientes.sapSelectedId);
      }
...
async function fn_testar_sap() {
  ...
  try {
    ...
    const resp = await fetch((prefix || '') + '/api/sap/testar-conexao/', {
      ...
      body: JSON.stringify({ cod_cliente: codCliente, sap_id: sapId || null })
    });
    const data = await resp.json();
    if (data.sucesso) {
...
function fn_init_eventos_modal_cliente() {
  ...
  const modalClienteUpd = document.getElementById('modalClienteUpd');
  if (modalClienteUpd) {
    modalClienteUpd.addEventListener('click', function(event) {
      const actionElement = event.target.closest('[data-action]');
      if (!actionElement) return;

      const { action } = actionElement.dataset;
      if (action === 'remover-solucao') {
        fn_remover_solucao(actionElement.dataset.solucao);
      } else if (action === 'remover-grupo-cliente') {
        fn_remover_grupo_cliente(Number(actionElement.dataset.grupoId));
      }
    });

    modalClienteUpd.addEventListener('change', function(event) {
      const target = event.target;
      if (target.matches('[data-action="alternar-solucao-status"]')) {
        fn_toggle_solucao_status(target.dataset.solucao);
      }
    });
  }

  const btnAdicionarSolucao = document.getElementById('btn-adicionar-solucao');
  if (btnAdicionarSolucao) {
    btnAdicionarSolucao.addEventListener('click', fn_adicionar_solucao);
  }

  const btnAdicionarGrupoCliente = document.getElementById('btn-adicionar-grupo-cliente');
  if (btnAdicionarGrupoCliente) {
    btnAdicionarGrupoCliente.addEventListener('click', fn_adicionar_grupo_cliente);
  }

  // Botão Nova conexão SAP
  const btnNovaConexaoSap = document.getElementById('btn-nova-conexao-sap');
  if (btnNovaConexaoSap) {
    btnNovaConexaoSap.addEventListener('click', fn_nova_conexao_sap);
  }

  // Select de conexões SAP
  const sapConnectionsSelect = document.getElementById('sap_connections_select');
  if (sapConnectionsSelect) {
    sapConnectionsSelect.addEventListener('change', function () {
      fn_selecionar_sap_connection(this.value);
    });
  }
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', fn_init_eventos_modal_cliente);
} else {
  fn_init_eventos_modal_cliente();
}
