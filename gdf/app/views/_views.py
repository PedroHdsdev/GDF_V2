...
def fn_api_consulta_fiscal_material(request):
    ...
    try:
        ...
        return JsonResponse({'sucesso': False, 'mensagem': 'Erro inesperado ao consultar os dados fiscais.'}, status=500)


@login_required(login_url='Login')
@requer_acesso_subsolucao('Con_Fiscal_Material', redirect_on_deny=False)
@require_http_methods(["GET"])
def fn_api_consulta_sap_fiscal_material(request):
    cod_cliente = request.session.get('cod_cliente', None)
    if not cod_cliente:
        return JsonResponse({'sucesso': False, 'mensagem': 'Cliente nao identificado'}, status=403)
    ...
    filtros = {
        'chave_acesso': (request.GET.get('chave_acesso') or '').strip(),
        'cod_material': (request.GET.get('cod_material') or '').strip(),
        'fornecedor': (request.GET.get('fornecedor') or request.GET.get('cod_fornecedor') or '').strip(),
    }

    try:
        result = consultar_fiscal_material(filtros)
        items = [normalizar_item_para_tabela(item) for item in (result.items or [])]
        return JsonResponse({
            'sucesso': True,
            'items': items,
            'mensagem': (result.mensagem or '').strip() or 'Consulta SAP realizada com sucesso.',
        }, status=200)
    except Exception as exc:
        logger.exception('CFM SAP erro inesperado cliente=%s filtros=%s', cod_cliente, filtros)
        return JsonResponse({'sucesso': False, 'mensagem': str(exc)[:500]}, status=500)
