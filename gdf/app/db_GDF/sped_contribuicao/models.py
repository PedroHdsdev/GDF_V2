...
class SpedContribuicaoArquivo(models.Model):
    ...
    def __str__(self):
        return f"SPED Contribuição - {self.nome_arquivo or self.id_arquivo}"


class SpedContribuicaoReg_0000(models.Model):
    """Registro 0000 - Abertura do arquivo digital."""
    id = models.AutoField(primary_key=True)
    arquivo = models.ForeignKey(SpedContribuicaoArquivo, on_delete=models.CASCADE, related_name='reg_0000')
    ...
    class Meta:
        managed = True
        db_table = '"sped_contribuicao"."sped_reg_0000"'
        indexes = [models.Index(fields=['arquivo'], name='sc_reg_0000_arquivo')]


class SpedContribuicaoReg_0001(models.Model):
    """Registro 0001 - Abertura do bloco 0."""
    id = models.AutoField(primary_key=True)
    arquivo = models.ForeignKey(SpedContribuicaoArquivo, on_delete=models.CASCADE, related_name='reg_0001')
    linha = models.IntegerField(blank=True, null=True)
    ...
    class Meta:
        managed = True
        db_table = '"sped_contribuicao"."sped_reg_0001"'
        indexes = [models.Index(fields=['arquivo'], name='sc_reg_0001_arquivo')]


class SpedContribuicaoReg_0005(models.Model):
    """Registro 0005 - Dados complementares do informante."""
    id = models.AutoField(primary_key=True)
    arquivo = models.ForeignKey(SpedContribuicaoArquivo, on_delete=models.CASCADE, related_name='reg_0005')
    linha = models.IntegerField(blank=True, null=True)
    ...
    class Meta:
        managed = True
        db_table = '"sped_contribuicao"."sped_reg_0005"'
        indexes = [models.Index(fields=['arquivo'], name='sc_reg_0005_arquivo')]


class SpedContribuicaoReg_0150(models.Model):
    """Registro 0150 - Cadastro do participante."""
    id = models.AutoField(primary_key=True)
    arquivo = models.ForeignKey(SpedContribuicaoArquivo, on_delete=models.CASCADE, related_name='reg_0150')
    linha = models.IntegerField(blank=True, null=True)
    ...
    class Meta:
        managed = True
        db_table = '"sped_contribuicao"."sped_reg_0150"'
        indexes = [models.Index(fields=['arquivo'], name='sc_reg_0150_arquivo'), models.Index(fields=['cod_part'], name='sc_reg_0150_codpart')]


class SpedContribuicaoReg_0190(models.Model):
    """Registro 0190 - Identificação das unidades de medida."""
    id = models.AutoField(primary_key=True)
    arquivo = models.ForeignKey(SpedContribuicaoArquivo, on_delete=models.CASCADE, related_name='reg_0190')
    linha = models.IntegerField(blank=True, null=True)
    ...
    class Meta:
        managed = True
        db_table = '"sped_contribuicao"."sped_reg_0190"'
        indexes = [models.Index(fields=['arquivo'], name='sc_reg_0190_arquivo')]


class SpedContribuicaoReg_0200(models.Model):
    """Registro 0200 - Cadastro do item."""
    id = models.AutoField(primary_key=True)
    arquivo = models.ForeignKey(SpedContribuicaoArquivo, on_delete=models.CASCADE, related_name='reg_0200')
    linha = models.IntegerField(blank=True, null=True)
    ...
    class Meta:
        managed = True
        db_table = '"sped_contribuicao"."sped_reg_0200"'
        indexes = [models.Index(fields=['arquivo'], name='sc_reg_0200_arquivo'), models.Index(fields=['cod_item'], name='sc_reg_0200_coditem')]


class SpedContribuicaoReg_C001(models.Model):
    """Registro C001 - Abertura do bloco C."""
    id = models.AutoField(primary_key=True)
    arquivo = models.ForeignKey(SpedContribuicaoArquivo, on_delete=models.CASCADE, related_name='reg_c001')
    linha = models.IntegerField(blank=True, null=True)
    ind_mov = models.CharField(max_length=1, blank=True, null=True)
    data_criacao = models.DateTimeField(auto_now_add=True)
    ...
    class Meta:
        managed = True
        db_table = '"sped_contribuicao"."sped_reg_c001"'
        indexes = [models.Index(fields=['arquivo'], name='sc_reg_c001_arquivo')]


class SpedContribuicaoReg_C100(models.Model):
    """Registro C100 - Documento fiscal (blocos C/D em EFD Contribuições)."""
    id = models.AutoField(primary_key=True)
    arquivo = models.ForeignKey(SpedContribuicaoArquivo, on_delete=models.CASCADE, related_name='reg_c100')
    linha = models.IntegerField(blank=True, null=True)
    ...
    class Meta:
        managed = True
        db_table = '"sped_contribuicao"."sped_reg_c100"'
        indexes = [models.Index(fields=['arquivo'], name='sc_reg_c100_arquivo'), models.Index(fields=['chv_nfe'], name='sc_reg_c100_chv')]


class SpedContribuicaoReg_C170(models.Model):
    """Registro C170 - Item do documento fiscal."""
    id = models.AutoField(primary_key=True)
    arquivo = models.ForeignKey(SpedContribuicaoArquivo, on_delete=models.CASCADE, related_name='reg_c170')
    c100 = models.ForeignKey(
        SpedContribuicaoReg_C100,
        on_delete=models.CASCADE,
    ...
    class Meta:
        managed = True
        db_table = '"sped_contribuicao"."sped_reg_c170"'
        indexes = [models.Index(fields=['arquivo'], name='sc_reg_c170_arquivo'), models.Index(fields=['c100'], name='sc_reg_c170_c100')]


class SpedContribuicaoReg_C190(models.Model):
    """
    Registro C190 - EFD Contribuições: Consolidação de NF-e por item (aquisições/devoluções com crédito).
    Diferente do C190 Fiscal (analítico ICMS). Tem COD_ITEM como principal; detalhes PIS/COFINS em C191/C195.
    """
    id = models.AutoField(primary_key=True)
    arquivo = models.ForeignKey(SpedContribuicaoArquivo, on_delete=models.CASCADE, related_name='reg_c190')
    c100 = models.ForeignKey(
        SpedContribuicaoReg_C100,
        on_delete=models.CASCADE,
    ...
    linha = models.IntegerField(blank=True, null=True)
    cod_item = models.CharField(max_length=60, blank=True, null=True, db_index=True)  # Principal no modo consolidado
    cfop = models.CharField(max_length=4, blank=True, null=True)
    # Campos do modo consolidado (C191/C195 agregados ou C190 com totais)
    cst_pis = models.CharField(max_length=2, blank=True, null=True)
    vl_bc_pis = models.DecimalField(max_digits=15, decimal_places=2, blank=True, null=True)
    vl_pis = models.DecimalField(max_digits=15, decimal_places=2, blank=True, null=True)
    cst_cofins = models.CharField(max_length=2, blank=True, null=True)
    ...
    # Campos compatibilidade (quando C190 vem após C100 em fluxo detalhado)
    cst_icms = models.CharField(max_length=3, blank=True, null=True)
    aliq_icms = models.DecimalField(max_digits=15, decimal_places=2, blank=True, null=True)
    vl_bc_icms = models.DecimalField(max_digits=15, decimal_places=2, blank=True, null=True)
    vl_icms = models.DecimalField(max_digits=15, decimal_places=2, blank=True, null=True)
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=['arquivo'], name='sc_reg_c190_arquivo'),
            models.Index(fields=['c100'], name='sc_reg_c190_c100'),
            models.Index(fields=['cod_item'], name='sc_reg_c190_coditem'),
        ]


class SpedContribuicaoReg_D100(models.Model):
    """Registro D100 - Documento de transporte."""
    id = models.AutoField(primary_key=True)
    arquivo = models.ForeignKey(SpedContribuicaoArquivo, on_delete=models.CASCADE, related_name='reg_d100')
    linha = models.IntegerField(blank=True, null=True)
    ind_oper = models.CharField(max_length=1, blank=True, null=True)
    ind_emit = models.CharField(max_length=1, blank=True, null=True)
    cod_part = models.CharField(max_length=60, blank=True, null=True)
    ...
    class Meta:
        managed = True
        db_table = '"sped_contribuicao"."sped_reg_d100"'
        indexes = [models.Index(fields=['arquivo'], name='sc_reg_d100_arquivo'), models.Index(fields=['chv_cte'], name='sc_reg_d100_chv')]


class SpedContribuicaoRegistro(models.Model):
    """Registros sem tabela específica: tipo, linha e campos em JSON."""
    id = models.AutoField(primary_key=True)
    arquivo = models.ForeignKey(SpedContribuicaoArquivo, on_delete=models.CASCADE, related_name='registros')
    ...
    class Meta:
        managed = True
        db_table = '"sped_contribuicao"."sped_registro"'
        indexes = [
            models.Index(fields=['arquivo'], name='sc_registro_arquivo'),
            models.Index(fields=['arquivo', 'registro'], name='sc_registro_arq_reg'),
        ]
