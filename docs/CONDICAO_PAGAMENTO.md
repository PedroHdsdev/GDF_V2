...
Referência de códigos: `gdf/json/Tipo_pagamento.json` (ex.: 01=Dinheiro, 03=Cartão de Crédito, 17=PIX).
...
| O quê | Onde |
|-------|------|
| Modelos CondicaoPagamentoLote, CondicaoParam | `gdf/app/db_GDF/reprocessamento/models.py` |
| condicao_pagamento_da_nfe, tipo_pagamento_da_nfe, _condicao_sap_da_param, gerar_condicoes_pagamento_lote | `gdf/app/classes/Reprocessamento.py` |
| _salvar_condicao_param_se_nao_existir (carga NFe) | `gdf/app/classes/CargaXml.py` |
| enviar_condicoes_pagamento_sap (RFC) | `gdf/app/classes/SapRfc.py` |
| APIs (gerar, listar, atualizar retorno, enviar SAP, condicao-param listar/atualizar) | `gdf/app/views.py` |
| URLs das APIs | `gdf/config/urls.py` |
| Tipos de pagamento (tPag) | `gdf/json/Tipo_pagamento.json` |
| Painel (HTML/JS) | `gdf/templates/app`, `gdf/static/js/Script_ReprocessamentoPainel.js` |
